#!/usr/bin/env python3
#python3 main.py < flag.txt > output.txt
flag = bytearray(input().encode())

#Note to self: Remember to hide later!
...
bramble_keys = [[252, 124, 88, 199, 15, 171, 250, 101, 132, 103, 151, 251, 237, 14, 91, 118, 8, 7, 73, 14, 222, 212, 172, 46, 99, 103, 112, 207, 133, 210, 145, 226, 29, 182, 184, 125, 162, 50, 80, 116, 89, 73, 184, 78, 211, 93, 12, 89, 27, 235, 84, 64, 164, 180, 254, 211, 37, 254, 108],
...
for i in range(3):
    for j in range(len(flag)):
        tmp[scramble_wheels[i][j]] = out[j] ^ bramble_keys[i][j]
    out = tmp[:]

print(f'[{", ".join(f"{x}" for x in out)}]')
